...
const ArticleList: React.FC = () => {
    ...
    //             <LazyImage className={`${className}-icon`} src={`/icon/${item.icon}.webp`} alt="" />
    //         </div>
    //     ));
    // };
    ...
    const renderArticles = (): JSX.Element[] => {
        return article.map((item) => (
            <div className="articles-flex-items" key={item.id}>
                ...
                <LazyImage className="cover" src={`/img/bg/${randomNumber(1, 14, 0)}.webp`} alt="" onClick={() => handleLink(item.id, false)} />
